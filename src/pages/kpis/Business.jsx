...
const DoubleCircle = ({ value, total }) => {
  const outerDiameter = 110;

  const innerDiameter = Math.max(30, (value / total) * outerDiameter);

  return (
    <div className="relative flex items-center justify-center my-6">
      {/* Outer Circle (fixed for all KPIs) */}
      <div
        className="relative rounded-full bg-[#4ABF8A33] flex items-center justify-center"
        style={{
          width: outerDiameter,
          height: outerDiameter,
        }}
      >
        {/* Inner Circle (value / total) */}
        <div
          className="absolute bottom-0 rounded-full bg-[#4ABF8A] flex flex-col items-center justify-center transition-all duration-500"
          style={{
...
export default function Business() {
  ...
  const maxNationalityValue = Math.max(...byNationality.map((i) => i.value));
  ...
  const revenueJobsEnd = rows.findIndex(
    (r, i) =>
      i > revenueJobsStart &&
      ["Jobs subsidized - by type", "Number of companies"].includes(r._1),
  );
    ...
    <div className="w-full">
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-4">
          {[jobs, revenue, emirati].map((kpi) => (
            ...
            >
              ...
              <DoubleCircle
                value={toNumber(kpi?._2)}
                total={toNumber(kpi?._3)}
              />
        ...
        <div className="grid grid-cols-2 gap-4 mt-[16px] rounded-[16px]">
          <div>
            <h1 className="text-[21px] text-white mb-1">
              Average cost per FTE (AED)
            </h1>
            <div
            ...
            >
              <h2 className="text-[#1FCC7D] font-bold text-[32px]">
                {toNumber(avgCost?._2) / 1000}k
              </h2>
              <p className="text-[14px] font-normal mt-[24px]">
                Target: {toNumber(avgCost?._3) / 1000}k
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            >
              <h2 className="text-[#FF4D50] font-bold text-[32px]">
                {jobsSupported?._2}
              </h2>
              <p className="text-[14px] font-normal mt-[24px]">
                Target: {jobsSupported?._3}
              </p>
            </div>
          </div>
        </div>
                    ...
                    <div className="flex w-full items-center text-center gap-2">
                      <div
                        ...
                        style={{
                          width: `${item.pct}%`,
                          background:
                            TYPE_BG[index] || TYPE_BG[TYPE_BG.length - 1],
                        }}
                      />
                      <span>
                        {item.value} ({item.pct}%)
              ...
              {nationalityData.map((item) => (
                <div key={item.name} className="mb-3">
                  <div className="flex items-center text-[10px] gap-2 text-white">
                    {/* Label */}
                    <span className="w-[70px] flex items-center gap-1">
                    ...
                    <div className="flex w-full items-center gap-2">
                      ...
                      {/* Value */}
                      <span className="min-w-[80px] text-right text-white/80">
                        {item.value} ({item.pct}%)
                      </span>
                    </div>
                  </div>
        ...
        <div className="mt-[16px]">
          ...
          >
            <div className="grid grid-cols-3 gap-4 mt-[10px]">
              {companies.map((c) => (
                <div key={c.type}>
                  <h2 className="text-[12px] text-[#85939F]">
                    {c.type} companies 2025 target:{" "}
                    <span className="text-[15px] text-[#FFFFFF]">
                      {c.target}
                    </span>
                  </h2>
                  <div
                    className="rounded-[16px] p-1 mt-1 text-white"
                    style={{
                      background:
                        "linear-gradient(180deg, rgba(255,255,255,0.06) 0%, rgba(255,255,255,0.03) 100%)",
                      backdropFilter: "blur(108px)",
