...
const SHEET_URL =
  "https://docs.google.com/spreadsheets/d/1zwMlxUzU9mQf20IOMYP9cCcnfDcqRYoO/export?format=csv&gid=963986118";

function parseNextSteps(rows) {
  const startIndex = rows.findIndex((r) => r[""] === "Activity");
  const endIndex = rows.findIndex((r) => r[""] === "KPI");

  if (startIndex === -1) return [];

  return rows
    .slice(startIndex + 1, endIndex !== -1 ? endIndex : undefined)
    .filter((r) => r[""] && r._1 && r._2 && r._3 && r._4)
    .map((r) => ({
      activity: r[""],
      owner: r._1,
      date: r._2,
...
function parseKeyCompanies(rows) {
  ...
  const startIndex = rows.findIndex((r) => r[""] === "Key Companies");
  const endIndex = rows.findIndex((r) => r[""] === "Activity");
    ...
    .forEach((row) => {
      ...
      if (label === "% completion in each stage" && current) {
        current.items = Array.from({ length: STAGES_COUNT }, (_, i) => ({
          value: Number((row[`_${i + 1}`] || "0").replace("%", "")),
        }));
      }
...
function parseKPIs(rows) {
  const startIndex = rows.findIndex((r) => r[""] === "KPI");
  if (startIndex === -1) return [];

  return rows
    .slice(startIndex + 1)
    .filter((r) => r[""] && r._1 !== undefined)
    .map((r) => ({
      name: r[""],
      actual: Number(r._1),
      monthlyTarget: Number(r._2),
      yearlyTarget: Number(r._3),
      gapMonthly: Number(r._4),
      percentMonthly: r._5,
      gapYearly: Number(r._6),
    }));
}

function parseRoadblock(rows) {
  const index = rows.findIndex(
    (r) => r[""]?.trim() === "Ecosystem roadblocks"
  );
...
export default function Outreach({ onBack }) {
  ...
  useEffect(() => {
    fetch(SHEET_URL)
      ...
      .then((csv) => {
        ...
        const rows = parsed.data.filter((r) => r[""]);
  ...
  const mainKPI = kpis.filter((k) => k.name === "Large Companies in the Ecosystem");

  const supportingKPIs = kpis.filter((k) => k.name !== "Large Companies in the Ecosystem");
        ...
        <div className="space-y-3 max-w-[761px]">
          ...
          >
            ...
            <p className="text-[13px] text-white/70 max-w-[713px] leading-relaxed">
              Conduct targeted outreach initiatives to attract major game
              studios, publishers, and game ops centres to establish regional
          ...
          <GlassCard title="Key companies">
            ...
            <div className="rounded-[16px] border border-white/10 overflow-hidden">
              ...
              <div className="grid grid-cols-6 text-[11px] text-white/60">
                ...
                {[
                  "Shortlist of high priority companies",
                  "Conduct intitial outreach",
                  "Build business cases",
                  "Negotiate incentives packages",
                  "Onboarding",
                ].map((t, i) => (
                  <div
                    key={t}
